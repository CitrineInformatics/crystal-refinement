...
class OptimizerUtils:
    ...
    def score_compound_bonds(self, bonds, shelx_file, ml_model=None):
        """
        ...
        :return:
        """
        site_bond_scores = self.get_site_bond_scores(bonds, 1, ml_model)
        total_stoich = 0
    ...
    def get_bond_score(self, bond, ml_model=None):
        """
        ...
        """
        ideal_bond_length = self.get_ideal_bond_length(bond[0], bond[1], ml_model)
        # Calculate amount which bonds deviate from the expected bond length
        return -abs(bond[2] - ideal_bond_length)

    def get_bond_score2(self, bond, ml_model=None):
        """
        Scores the likelihood of a bond based on its deviation from an ideal bond length.
        :param bond: The bond to score
        :param ml_model: Whether to use an ml model to calculate the ideal bond length.
    ...
    def get_site_bond_scores(self, bonds, n_bonds=4, ml_model=None):
        """
        ...
        for bond in bonds:
            bond_score = self.get_bond_score(bond, ml_model)
    ...
    def site_mixing_priority(self, bonds, n_bonds=4, ml_model=None):

        # Get associated site index
        return map(lambda tup: (int(re.search("\d+", tup[0]).group(0)), tup[1]), self.get_site_bond_scores(bonds, n_bonds, ml_model))


    def get_ideal_bond_length(self, specie_name1, specie_name2, ml_model=None):
        """
        Scores the likelihood of a bond based on its deviation from an ideal bond length.
